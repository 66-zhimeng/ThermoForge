...
日期：2026-09-07。本文保留完整阶段设计，并在下表记录本批实现边界；未列为已实现的部分继续作为后续计划。本次更新不改变已保存的原始讨论稿，不启动新的付费模型研究。

## 已确定的模型配置

根据用户要求，新运行的主 Codex 与五个候选 Codex 默认统一使用 **GPT-6 Astra（`gpt-6-astra`）、最高思考强度 `ultra`、普通速度（关闭 Fast）**。运行时为实例和每轮请求明确指定普通速度，不依赖本机是否启用 Fast；旧实验的配置与历史证据保持原样，未记录速度时不追认其速度档位。

后续优化保留该模型与最高思考档位，优先减少重复上下文、工具往返和无依据的重复实验，并提高反馈与成果保存质量。不能通过自动降低思考强度、替换更小模型或启用 Fast 来宣称完成这些优化。

## 本批实现状态

| 方向 | 本批状态与边界 |
|---|---|
| 冻结研究目标 | 已实现 `objective_mode` 达标/优化/探索及自动判定、主指标、相对改善、证据审阅和停滞建议；阈值、实际执行、基线改善分别判定 |
| 共同基线 | 已实现预先指定内置 `baseline_model`，按同协议首次精确匹配的真实成功执行识别结果；尚无自动执行调度，缺基线保持未确认，不事后选择最优当基线；lab 源码身份尚不能跨轨迹冻结，因此不支持 lab 共同基线 |
| 证据质量 | 缺物理、延迟或外推证据保持未确认；A/C/auto 验收与 validate 搜索分开。默认审阅要求发现包含解释与限制，不是独立复验或统计检验 |
| 持续事实交付 | 已实现实验结算、分析和报告保存、服务收尾时导出 checkpoints；事实简报、实验 JSON、当前报告与流程图保留缺项，系统汇总不计为智能体报告 |
| 反馈与结题 | 已接入按目标状态收尾、实验后的分析与滚动报告阶段；目标满足并有审阅记录后不机械耗尽额度，仍需智能体保存报告和停止理由 |
| 紧凑上下文 | 已实现回合直接投递新反馈/未处理消息、紧凑历史、游标、字段投影、按 ID 与长文本分页；原始记录继续留存，独立策略权限不变 |
| 用量观测 | 已实现模型累计计数去重增量、工具响应字节数与耗时；这些数据不直接代表费用，尚无新真实运行证明节省幅度 |
| 实验诊断 | 已实现真实 validate NMBE 与偏差方向；训练面与训练—验证差距没有可用实测时明确缺测，不挪用留出数据。设备/时间/负荷切片、复杂残差诊断待实现 |
| 后续研究机制 | 原子登记想法和提案、自动共同基线执行、完整 EvoX 元进化与新付费科学对照尚未实现 |

使用入口与字段见 [V2 使用指南](../docs/v2-getting-started.md)。持续产物目录为 `<TF_RESEARCH_ROOT>/v2/runs/<run_id>/checkpoints/`；`latest.json` 指向当前事实与工件哈希，`jobs/` 保留实验事实，`team-flow.html` 可追踪已登记路线。额度耗尽仍可能缺少智能体解释或停止决定，系统不会用模板补造这些成果。
...
本批软件闭环已有 399 项 V2 回归及桌面/手机检查通过，详见[成果优化验收记录](../docs/v2-outcome-optimization-validation.md)。新增调度以模拟 Codex 验证，实验工具包含真实 CPU 实验；未新增付费研究运行。后续真实研究需继续验证正常报告、预算耗尽和中断时的交付质量，以及达标后复核、无改善、实现失败等停止依据。比较优化前后的输入量、工具往返、完成实验数与报告覆盖，不能只比较最优验证分数。
