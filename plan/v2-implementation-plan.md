...
更新日期：2026-09-06。状态：按用户最新澄清，首期目标确定为独立 ThermoForge 软件在后台隐藏启动 1 个主 Codex 与 5 个候选 Codex；各自独立进程、会话和研究循环，通过软件通信，无需独立窗口。处于规划阶段，尚未实施运行时改造或启动这些实例。
...
代码基线仍为当前 `harness-schemes` 工作树，包含既有未提交更新。V1/V2 是产品演进称呼；正式冻结 V1 时记录最终提交、环境、数据和评价协议，不把当前工作树默认为已经发布的版本。
...
本轮没有对配置中的实际模型端点发请求，不能断言该服务商的兼容性或模型主动性有缺陷。文本 ping 不等于工具能力验证。更换 API 地址不会自动增加会话管理、研究调度或工具执行循环。
...
接入依据已做只读核验：本机 `codex-cli 0.151.0` 的帮助包含 `app-server`、`exec --json`、恢复会话及 `--remote` 终端连接能力；未启动模型任务。官方 Python SDK 可程序控制本地 Codex，发布包包含固定版本运行时。实施优先验证 Python SDK 与独立实例映射，必要时直接使用 App Server 协议；固定版本并验证进程所有权，不能因为创建六个 SDK 对象就认定已经有六个独立后台进程。[Codex SDK](https://learn.chatgpt.com/docs/codex-sdk)
...
接入原型优先验证 SDK 管理的本地后台进程与程序通信；不再把终端连接或官方桌面窗口管理列为前置条件。Windows 启动器默认隐藏运行，持续收集协议事件和错误日志，避免研究依赖可见窗口或人工输入“继续”。缺少认证或必要用户输入时，将实际原因与等待状态传回软件和副驾驶，不让后台进程静默卡在不可见的交互界面。查看单个实例通过统一状态、事件和报告接口完成；将来需要调试窗口时另行提供，不影响首期验收。
...
本轮按用户回复确定六个独立 Codex 进程在后台隐藏启动，删除独立窗口及其原型验收要求，保留程序通信、独立方案/报告及接受更多 token 的既定要求。只更新规划文档，不改运行代码、不启动模型调用或实验。原始讨论稿及已注明被替代的历史计划继续保留。
