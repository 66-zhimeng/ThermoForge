...
### I-48 F6 COP 量纲问题——已撤销（高温离心式冷机）

**背景**：[data-survey §F6](./data-survey.md) 测得系统 COP 中位数 9.99~11.31，探查时按常规水冷离心机（7 °C 供水，COP 5~7）判定为「物理不可能」。2026-08-09 数据提供方确认：该站点为**高温离心式冷机**，冷冻水供水中位数 17.65 °C，此工况下 COP 9~11 物理合理（Carnot 上限复核：蒸发侧 ~290 K、温升 12~16 K，Carnot COP ≈ 18~24，实测约为其 50%）。F6 结论作废，物理建模路线的前置阻断解除。
