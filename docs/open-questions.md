...
**已回答（2026-08-09，数据提供方确认）**：数据来自**现场**，由现场系统导出为 Excel，15 分钟级。与 [data-survey §F9](./data-survey.md) 的仿真迹象对照后的结论是：底层驱动序列（总管温度流量、环境参数、设备功率/电流）为现场实测，而工作簿中约 296 万个公式单元格是**在 Excel 内对实测序列做派生计算**的结果（单机温度、`load`、冷却水流量等）。因此「仿真 vs 现场」的答案不是二元的：实测序列可信，但派生列的物理关系是 Excel 公式，建模输入必须区分 `source_kind`（measured / derived）——这一区分已在导入管线与 DD-16 白名单中强制执行。

**遗留**：公开授权与脱敏范围仍待确认；F6 的 COP 量纲问题不因数据来源确认而消失（见 I-48）。
