...
**修订（2026-08-17）：方案 C 已落地为「模型实验室」，且不设人工审批。**
内置模型闭集穷尽后无路可走，正是上述复审触发条件。落地形态见
`thermoforge_research.model_lab`：Agent 写单文件模型模块 → AST 源扫描 →
子进程五连检（接口/拟合预测/save 契约/往返一致/同种子位级确定）→ 版本化
不可变入库 → 直接以 `category=lab` 进正式实验。

与最初设想的差别有两处，都是刻意的：

1. **不"固化为新工具"，而是固化为带版本的模型模块**。固化成工具意味着每个
   新方程都要改代码发版，研究节奏又回到工程节奏上——正是本决策的已知失败
   模式。模块化 + 内容哈希版本化能达到同样的可复现性：源码快照冻结在实验
   目录与发布包里，历史实验不受后续版本影响。
2. **门禁是机器判定，不是人工审批**。可复现性由五连检（尤其位级确定性）与
   环境锁保证，这些机器都能判；而"这个建模假设好不好"本来就该由证据淘汰，
   排一道人工队列既拦不住坏假设，也让 Agent 无法自己完成"跑→看指标→改代码"
   的闭环。对照：预处理规则改的是**数据**，仍然必须 `actor=human` 审批。

**代价（新增）**：静态扫描成为候选代码执行前唯一的自动关卡，它是粗筛不是
内核沙箱——候选代码在本机子进程里以当前用户身份执行。这一风险敞口是明知
并接受的：Agent 写的代码，信任级别等同于本机运行的其它代码。`deprecate` 是
事后否决口，不在关键路径上。
...
**连带影响**：DD-08 的物理模型选型需推迟——实测系统 COP 中位数 9.99~11.31，超出水冷离心机组的物理范围，须先解决量纲/标定问题（[data-survey §F6](./data-survey.md)）。（2026-08-09 更新：F6 已撤销——站点为高温离心式冷机，COP 9~11 物理合理，DD-08 的前置阻断解除。）
...
## DD-17 下游双消费画像：仿真用动态模型 + 寻优用可微静态模型

**状态**：已采纳（2026-08-09 与需求方确认；回答 [缺口 G1](./gap-analysis.md)）

**问题**：模型的下游消费者是谁、需要什么形态的模型？（G1 原为完全空白）
...
|---|---|---|---|
| 仿真 | 动态模型 | **秒级** | 不要求可微 |
| 算法寻优 | 静态模型 | 稳态工况 | **必须可微且连续**（梯度类优化器直接调用） |

**选择与理由**：两类消费对应两条模型产品线，签名与约束需分别声明。寻优场景强制「可微连续」直接构成**模型选型约束**：纯树模型（XGBoost 等）输出是分片常数、不可微，只能用于仿真/预测用途，不得作为寻优模型发布；寻优模型应从线性/多项式、平滑物理模型或平滑网络中选择。model-package §4 已有「是否连续、是否可微」的声明字段，该字段从建议项升级为**寻优模型的发布门禁项**。

**代价**

- 同一研究目标可能需要训练并维护两个模型变体（动态仿真用 + 静态寻优用），研究预算与验证工作量增加。
- 秒级动态模型依赖 G7（动态/时序建模契约）——该缺口此前被列为第二梯队，现因仿真需求提前为现实约束：签名 `history_required`（C4）与状态空间表达需要定稿。
- 树模型在寻优场景被排除，会牺牲一部分拟合精度（本次切片中树模型并未占优，代价可接受）。

**复审触发**：寻优器实际接口确认后（调用频率、批量、失效降级策略）；若寻优改用无梯度方法（如贝叶斯优化、启发式），可微性约束可放宽。
...
flowchart TD
    ...
    DD17["DD-17 下游双消费画像"] -.约束寻优模型选型.-> DD07
```
