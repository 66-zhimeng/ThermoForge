...
5. **V2 自主研究走任务级接口**：先 `tf_v2_prepare` 发现真实目标、数据和配置；
   用户已要求启动且输入齐备就 `tf_v2_start`，保存 run_id 与幂等键。
   之后使用 `tf_v2_status/events/control/report` 查询、调整、暂停恢复和读取报告。
   软件在后台管理主 Codex 与五个候选，网页关闭后继续，不需逐轮发“继续”。
   查询不代表启动授权，预算及指导变更以服务返回的实际版本与状态为准。
